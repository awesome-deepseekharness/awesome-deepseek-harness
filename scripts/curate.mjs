...
function buildPrompt({ pr, issue, preChecks }) {
  ...
  return [
    ...
    ``,
    `Task: Do a quick health audit (README tables, duplicates, star drift sample), and if GH_PR/GH_ISSUE is set, preprocess that PR/Issue per curator.md. For any new project (owner/repo), you MUST verify with BOTH webfetch AND kitesurf browser (chrome-devtools-mcp via wss://kitesurf.cloudflare.app/devtools/browser) — fetch repo page, check README for dsh install, verify dsh-plugin topic via gh api, and search for the project to confirm it's real. Then provide a maintainer review opinion (Approve / Request changes / Needs discussion) with confidence and suggested comment body (friendly, in PR language, ping @hdjekuue if needed). Always overwrite curator-report.md with the required structure including Preliminary Checks and Maintainer Review Opinion.`,
    ``,
    `Toolbox hints (MUST use BOTH webfetch AND kitesurf):`,
    `- Use gh api for GitHub data, curl + jq for APIs, and for repo verification ALWAYS run BOTH webfetch (raw HTML) and kitesurf browser (rendered + screenshots) in parallel; cite both Sources. For discovery, use websearch plus both webfetch and kitesurf on top hits.`,
    `- Use opencode public provider (https://opencode.ai/zen/v1, apiKey public) — you are already on a free model via traversal.`,
    `- kitesurf MCP is available as local MCP "kitesurf" (command: npx chrome-devtools-mcp --wsEndpoint=wss://kitesurf.cloudflare.app/devtools/browser) — ALWAYS use it to open https://github.com/owner/repo (and demo URLs) even if webfetch succeeded; do NOT treat it as fallback-only.`,
    ``,
