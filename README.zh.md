...
| [weibaohui/dsh-smart-title](https://github.com/weibaohui/dsh-smart-title) | 会话智能标题：每轮对话结束后用一次独立的辅助 LLM 调用对「用户消息+助手回答」的完整转写做总结，标题跟随会话真实主题而不是复述第一句话；首条消息即时生成标题、内置标题失败在后续轮次自动重试、用户手动改名绝不被覆盖、自动跳过子代理与 fork 会话。`dsh plugin add @weibaohui/dsh-smart-title` 即装 | 3 |
