...
| [weibaohui/dsh-git-server](https://github.com/weibaohui/dsh-git-server) | Git server: embeds ts-gogs (a TypeScript reimplementation of Gogs) and serves a full Git service on its own port — HTTP clone/push, web UI, issues/PRs/wiki — reusing user-management credentials, start/stop from the settings page. Installable via `dsh plugin add @weibaohui/dsh-git-server` | 1 |
